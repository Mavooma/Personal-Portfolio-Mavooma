...
function signUpNewsletter() {
    ...
    if (!form || !messageBox) return; // safety check

    try {
        // ✅ Show success message
        messageBox.textContent = "🎉 You’ve been signed up for the newsletter!";
        messageBox.className = "newsletter-message success show";
        ...
        messageBox.className = "newsletter-message error show";
...
function FunctionKitBotResponse(input) {
    ...
    if (qaDatabase[cleanedInput]) return qaDatabase[cleanedInput];
    for (let key in qaDatabase) {
        if (cleanedInput.includes(key)) return qaDatabase[key];
    }
